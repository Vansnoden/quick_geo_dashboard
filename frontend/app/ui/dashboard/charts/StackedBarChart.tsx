...
export default function StackedBarChart({ chart, dashboardId, interactiveFilters }: Props) {
                ...
                const fetchStackedData = async () => {
                        ...
                        try {
                                ...
                                const catResponse = await fetch(
                                        DASHBOARD_CHART_DISTINCT_VALS(Number(dashboardId)),
                                        {
                                                method: "POST",
                                ...
                                await Promise.all(
                                        validCategories.map(async (category: string) => {
        ...
        if (data.length === 0) return <Card className="text-gray-500 p-4">No data available</Card>;
