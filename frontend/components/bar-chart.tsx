...
// Custom hook to convert interactive filter object to FilterCondition array
const useFilterConditions = (interactiveFilters?: Record<string, any>) => {
    return useMemo(() => {
        ...
        for (const [key, val] of Object.entries(interactiveFilters)) {
            ...
            // Handle range filters (keys ending with _min or _max)
            if (key.endsWith('_min')) {
                ...
                if (maxVal !== undefined && maxVal !== '' && maxVal !== null) {
                    // Both min and max present -> between
                    conditions.push({
                        column,
                        operator: 'between',
                ...
                if (Array.isArray(val) && val.length > 0) {
                    ...
                    // Handle range as object (if passed as {min, max} - backup)
